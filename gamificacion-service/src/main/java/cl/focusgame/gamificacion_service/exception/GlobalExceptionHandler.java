...
public class GlobalExceptionHandler {
    ...
    // Falla de comunicacion con productividad-service, progreso-service o recompensa-service (Feign)
    @ExceptionHandler(FeignException.class)
    public ResponseEntity<ErrorResponse> feign(FeignException ex, HttpServletRequest req) {
        log.error("Error en llamada remota: status={}", ex.status());
        return build(HttpStatus.BAD_GATEWAY,
                "No se pudo contactar a un servicio remoto", req, List.of());
    }
